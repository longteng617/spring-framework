...
public class ContextLoader {
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		...
		try {
			...
			// <6> 记录到 currentContext 或 currentContextPerThread 中,差异在于类加载器的不同。
			// 目前这两个变量，在销毁 Spring WebApplicationContext 容器时会用到。暂时还看不太出其它的用途。
	...
	/**
	 * <1> 处，如果 wac 使用了默认编号，则重新设置 id 属性。默认情况下，我们不会对 wac 设置编号，所以会执行进去。
	 * 		而实际上，id 的生成规则，也分成使用 contextId 在 <context-param /> 标签中设置，和自动生成两种情况。
	 * 		默认情况下，会走第二种情况。
	 * <2> 处，设置 wac 的 ServletContext 属性。
	 * <3> 处，设置 context 的配置文件地址。
	 * <context-param>
